...
public class Digraph {
    ...
    /**
     * Returns the vertices that point to the given vertex, opposite
     * of adj. For use in academicplanner.
     * @param  v the vertex
     * @return the vertices pointing to vertex {@code v} in this digraph, as an iterable
     * @throws IllegalArgumentException unless {@code 0 <= v < V}
     */
    public Iterable<Integer> reverseAdj(int v) {
        validateVertex(v);
        Bag<Integer> reverseAdjList = new Bag<>();
        for (int i = 0; i < V; i++) {
            for (int w : adj[i]) {
                if (w == v) {
                    reverseAdjList.add(i);
                }
            }
        }
